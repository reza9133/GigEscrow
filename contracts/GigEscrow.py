...
#
# Error classification. _judge RAISES for anything that looks like an
# infrastructure hiccup rather than a fact about the deliverable itself, so
# GenVM's own leader-rotation/error-agreement machinery handles it — see
# _adjudicate's validator_fn and the try/except around _adjudicate() in
# dispute_milestone. Swallowing a timeout into a returned "INCONCLUSIVE"
# dict would make a transient outage look identical to "the network judged
# this deliverable and could not decide", which is a permanent, informative
# outcome recorded on a milestone. Those two situations are not the same
# thing and should not look the same to a client or freelancer reading the
# result.
...
# Real deliverables are frequently client-rendered apps (React/Vue/Next.js),
# not static pages. Without a render delay, GenVM can capture the page
# before its own scripts have painted anything, and every validator would
# be judging a blank shell rather than the actual deliverable.
...
def _judge(description: str, deliverable_url: str, note: str) -> dict:
	...
	Returns a small, fully self-describing dict ONLY on a genuine business
	outcome — a page that loaded fine and either does or does not satisfy
	the spec, or loaded fine but has nothing readable on it at all. Anything
	that looks like infrastructure trouble (the fetch itself failing, the
	LLM call itself failing, or the LLM responding with something that
	cannot be parsed as the requested JSON shape) is raised as a
	gl.vm.UserError instead, tagged with one of the ERR_* prefixes above, so
	the caller in _adjudicate can tell a network hiccup apart from an actual
	verdict and let GenVM's own retry path handle it.

	Only the `verdict` field of a successful result is ever compared between
	the leader and a validator (see `_adjudicate`) — `score` and `reasoning`
	are stored as evidence but never compared, because two independent LLM
	calls will almost never produce identical wording even when they agree
	completely on the outcome.
	"""
	try:
	...
	if content_len == 0:
		# The page loaded without error and simply has nothing on it --
		# a fact about this URL, not about the network. Recorded as a real
		# INCONCLUSIVE outcome, not raised.
		return {"verdict": VERDICT_INCONCLUSIVE, "score": 0,
			"reasoning": "", "fetch_note": "deliverable page returned no readable text",
			"content_len": 0}
	...
	if verdict not in (VERDICT_APPROVED, VERDICT_REJECTED, VERDICT_INCONCLUSIVE):
		raise gl.vm.UserError(ERR_LLM_MALFORMED + " verdict field missing or invalid")

	try:
		score = _clamp(int(data.get("score", 0)), 0, 100)
	except Exception:
		raise gl.vm.UserError(ERR_LLM_MALFORMED + " score field missing or invalid")
	...
	return {"verdict": verdict, "score": score, "reasoning": reasoning,
		"fetch_note": "", "content_len": content_len}
...
class GigEscrow(gl.Contract):
	...
	def _mark_settled(self, job: Job) -> None:
		"""O(1) job-completion check.

		milestone_count only ever grows, in fund_milestone; this counter
		only ever grows here, exactly once per milestone, at the moment it
		reaches a terminal status. Comparing the two is equivalent to
		re-scanning every milestone on the job on every settlement, without
		ever actually doing that scan — the old version of this function
		read every milestone on the job each time any one of them settled,
		which is fine for a handful of milestones but means a client who
		defines the maximum of MAX_MILESTONES_PER_JOB milestones pays for
		re-reading all of them on every single approval, dispute, or
		cancellation on that job.
		"""
		job.settled_milestone_count = u32(int(job.settled_milestone_count) + 1)
		if int(job.settled_milestone_count) >= int(job.milestone_count):
	...
	def _adjudicate(self, description: str, deliverable_url: str,
		...
		Raises gl.vm.UserError, tagged with one of the ERR_* prefixes from
		_judge, when the failure looks like infrastructure trouble rather
		than a fact about the deliverable. Both call sites catch this and
		respond gracefully instead of letting it propagate — see the
		money-safety note on dispute_milestone.
		"""
		def leader_fn() -> dict:
		...
		def validator_fn(leaders_res: gl.vm.Result) -> bool:
			if not isinstance(leaders_res, gl.vm.Return):
				...
				try:
					...
					if (leader_msg.startswith(_TRANSIENT_PREFIXES)
							and validator_msg.startswith(_TRANSIENT_PREFIXES)):
						# Both independently hit the same class of
						# infrastructure failure — agree, so the network
						# settles on "retry later" in one round instead of
						# forcing a leader rotation that will likely just
						# fail the same way again.
						return True
					return False
				except Exception:
					return False
				# The validator's own attempt succeeded where the leader's
				# failed — not a shared outage, so disagree and force a new
				# leader to try.
				return False
	...
	def create_job(self, freelancer: str, title: str) -> str:
		...
		job.settled_milestone_count = u32(0)
	...
	def dispute_milestone(self, job_id: int, index: int) -> str:
		...
		and any infrastructure failure while arbitrating (a fetch or
		LLM-provider outage) refunds the bond and returns
		{"ok": true, "verdict": "RETRY_LATER", ...} instead of raising.
		...
		Resolution:
		  ...
		  INCONCLUSIVE -> nothing is paid or refunded, the bond is returned,
		                  ...
		                  against either side.
		  RETRY_LATER  -> not a verdict at all -- _judge hit what looks like
		                  a transient infrastructure failure rather than a
		                  fact about the deliverable. Bond is returned,
		                  status is left exactly as it was, and nothing
		                  about this outcome is written to ai_verdict.
		"""
		sender = gl.message.sender_address
		...
		try:
			...
			# Infrastructure trouble (a fetch or LLM-provider outage, or a
			# malformed LLM response), not a verdict. The milestone is left
			# exactly as it was -- neither a REJECTED-challenge outcome nor
			# an INCONCLUSIVE business verdict is recorded -- and the bond
			# is returned so the same side can simply call this again
			# later. This is the payable-never-raises rule applied to a
			# case that only appears once the contract talks to real,
			# occasionally-unavailable infrastructure: see the module
			# docstring and the _judge/ERR_* comments above.
			self._pay(sender, bond)
			return json.dumps({"ok": True, "job_id": jid, "index": idx,
				"verdict": "RETRY_LATER", "status": str(ms.status),
				"paid_to_freelancer": "0", "refunded_to_client": "0",
				"bond_forfeited": False, "reason": str(getattr(e, "message", e))[:250]})
		...
		if verdict == VERDICT_APPROVED:
			...
			# INCONCLUSIVE -- not a terminal outcome, so settled_milestone_count
			# is deliberately left untouched: this milestone can still reach
			# a terminal status later via a retried dispute.
	...
	def preview_dispute(self, job_id: int, index: int) -> str:
		...
		try:
			...
			return json.dumps({"ok": True, "job_id": int(job_id), "index": int(index),
				"preview_verdict": "RETRY_LATER", "score": 0, "reasoning": "",
				"fetch_note": str(getattr(e, "message", e))[:250], "binding": False})
		obs = res if isinstance(res, dict) else {}
		verdict = str(obs.get("verdict", VERDICT_INCONCLUSIVE))
		return json.dumps({"ok": True, "job_id": int(job_id), "index": int(index),
	...
	def _job_json(self, job: Job) -> dict:
		return {"job_id": int(job.job_id), "client": str(job.client),
			...
			"settled_milestone_count": int(job.settled_milestone_count),
